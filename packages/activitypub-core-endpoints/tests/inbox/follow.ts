...
import { followActivityWithManualApproval, followActivityWithManualApprovalId, actor2Requests, actor1FollowersId, actor1Followers, actor2, actor2Id, actor1, actor1Id, followActivity, actor2Followers, actor2FollowersId, actor2RequestsId } from '../../test_data';
...
describe('Inbox', () => {
  describe('Follow Side Effect', () => {
    it('Handles Follow Activity when actor manually approves followers', async () => {
      ...
      await (handleFollow as unknown as (activity: AP.Follow) => Promise<void>).call({
        adapters: {
          db: {
      ...
      expect(`${collectionInsertedInto}`).toBe(actor2RequestsId);
