...
export const activityPub = (config: {
  pages: {
    login: () =>
      Promise<string>;
  ...
  async (req: IncomingMessage, res: ServerResponse, next: NextFunction) => {
    ...
    if (req.method === 'POST') {
      if (req.url === '/user') {
        await new UserPostEndpoint(req, res, config.adapters, config.plugins).respond();
        next();
        return;
      }

      if (req.url === '/sharedInbox') {
        await new SharedInboxPostEndpoint(req, res, config.adapters, config.plugins).respond();
        next();
        return;
      }

      if (req.url.startsWith('/actor/') && req.url.endsWith('/inbox')) {
        await new InboxPostEndpoint(req, res, config.adapters, config.plugins).respond();
        next();
        return;
      }

      if (req.url.startsWith('/actor/') && req.url.endsWith('/uploadMedia')) {
        await new UploadMediaPostEndpoint(req, res, config.adapters, config.plugins).respond();
        next();
        return;
      }

      if (req.url.startsWith('/actor/') && req.url.endsWith('/outbox')) {
        await new OutboxPostEndpoint(req, res, config.adapters, config.plugins).respond();
        next();
        return;
      }
    }
    ...
    if (req.method === 'GET') {
      ...
      if (req.url.startsWith('/.well-known/webfinger')) {
        await new WebfingerGetEndpoint(req, res, config.adapters, config.plugins).respond();
        next();
        return;
      }
