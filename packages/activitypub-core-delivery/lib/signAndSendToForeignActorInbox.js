...
async function signAndSendToForeignActorInbox(foreignActorInbox, actor, activity) {
    ...
    const { dateHeader, digestHeader, signatureHeader } = await this.adapters.crypto.getHttpSignature(foreignActorInbox, actor.id, await this.adapters.db.getPrivateKey(actor), convertedActivity);
