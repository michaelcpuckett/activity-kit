...
export async function getHttpSignature(
  ...
  const importedKey = await this.params.subtle.importKey(
    'pkcs8',
    privateKeyBuffer,
  ...
  const signer = await this.params.subtle.sign(
    { name: 'RSASSA-PKCS1-v1_5' },
    importedKey,
    encoder.encode(dateString + foreignDomain + foreignPathName),
  );

  if (entity) {
    const digestHash = await this.params.subtle.digest(
      { name: 'SHA-256' },
      encoder.encode(JSON.stringify(entity)),
    );
    const digestHashString = btoa(
      String.fromCharCode(...new Uint8Array(digestHash)),
    );
