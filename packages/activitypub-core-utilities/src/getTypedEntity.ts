...
export const getTypedEntity = (entity: {
  [key: string]: unknown;
}):
  | AP.Link
  | AP.Activity
  ...
  if (
    entity.type === AP.CollectionPageTypes.COLLECTION_PAGE ||
    (Array.isArray(entity.type) &&
      entity.type.includes(AP.CollectionPageTypes.COLLECTION_PAGE))
  ) {
    return entity as unknown as AP.CollectionPage;
  }

  if (
    entity.type === AP.CollectionPageTypes.ORDERED_COLLECTION_PAGE ||
    (Array.isArray(entity.type) &&
      entity.type.includes(AP.CollectionPageTypes.ORDERED_COLLECTION_PAGE))
  ) {
  ...
  for (const extendedObjectType of Object.values(AP.ExtendedObjectTypes)) {
    if (
      entity.type === extendedObjectType ||
