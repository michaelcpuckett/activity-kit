...
async function entityGetHandler(request, response, providedDatabaseService) {
    ...
    if (entity.type === types_1.AP.CollectionTypes.COLLECTION ||
        entity.type === types_1.AP.CollectionTypes.ORDERED_COLLECTION) {
    ...
    if ('likes' in entity && entity.likes instanceof URL) {
        ...
        if (foundLikesCollection &&
            foundLikesCollection.type === types_1.AP.CollectionTypes.ORDERED_COLLECTION) {
            const expandedLikes = await databaseService.expandCollection(foundLikesCollection);
            if (expandedLikes &&
                expandedLikes.type === types_1.AP.CollectionTypes.ORDERED_COLLECTION) {
    ...
    if ('shares' in entity && entity.shares instanceof URL) {
        ...
        if (foundSharesCollection &&
            foundSharesCollection.type === types_1.AP.CollectionTypes.ORDERED_COLLECTION) {
            const expandedShares = await databaseService.expandCollection(foundSharesCollection);
            if (expandedShares &&
                expandedShares.type === types_1.AP.CollectionTypes.ORDERED_COLLECTION) {
